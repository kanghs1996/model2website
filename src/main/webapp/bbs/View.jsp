	...
	<div class="table-responsive">
		<table class="table text-white align-middle">
			...
			<tfoot>
				<tr>
					<td colspan="2" scope="row" style="border-style: none; text-align: right;">
						<button class="btn btn-primary" id="like">추천</button>						
...
<script>
	...
	document.getElementById('like').addEventListener('click',function(e) {		
		if(!confirm("정말 추천하시겠습니까?")){
			...
			if(${ldao.likeIncrease(record.no, LOGIN) == 1} && likeClickCount == 0){
				alert('추천하였습니다');
				document.getElementsByTagName('span')[0].innerHTML = '추천수:${likeCount+1}'; 
				${ldao.close()};
				likeClickCount++;				
			}else{
				alert('이미 추천하였습니다');
