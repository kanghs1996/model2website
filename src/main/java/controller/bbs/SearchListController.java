...
public class SearchListController extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {	
		...
		// 타이틀이름 리퀘스트영역 저장
		req.setAttribute("title", "KOSMOSearchList");
		String id = req.getSession().getAttribute("LOGIN").toString();
		if (id.length() != 0) {
			BbsDAO bdao = new BbsDAO(getServletContext());
			// 페이징을 위한 로직 시작]
			// 전체 레코드수
			int totalRecordCount = bdao.getTotalRowCount(option, content);
			// 페이지 사이즈
			...
			map.put("end", end);
			// 페이징관련 로직 호출
			String pagingString = PagingUtil.pagingBootStrapStyleSearch(totalRecordCount, pageSize, blockPage, nowPage,
					req.getContextPath() + "/bbs/SearchList.kosmo?", option, content);
			List<BbsDTO> list = bdao.selectList(map, option, content);
			bdao.close();
			...
			// 이동
			req.getRequestDispatcher("/bbs/SearchList.jsp").forward(req, resp);
		} else {
