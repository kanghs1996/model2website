...
<main class="px-3">
	<div style="margin-bottom: 30px;">
		<h1>회원정보 수정</h1>
		<h4>비밀번호 변경을 원할시에만 입력해주세요</h4>
	</div>
	<form class="row gy-2 gx-3 align-items-center"
		action="<c:url value='/bbs/MemberEdit.kosmo'/>" method="post" name="form">
		<div class="mb-3 row">
			<label for="id" class="offset-1 col-3 col-form-label">아이디</label>
			<div class="col-7">
				<input readonly="readonly" type="text" class="form-control"
					name="id" id="id" value="${dto.id}">
			</div>
		</div>
		<div class="mb-3 row">
		...
		<div class="mb-3 row">
			...
			<div class="col-7" style="text-align: left;">
				<label for="gender1" class="form-check-label">남자</label> <input
					...
					value="man" ${dto.gender == 'man' ? 'checked="checked"' : ''}>
				<label for="gender2" class="form-check-label">여자</label> <input
					type="radio" class="form-check-input" name="gender" id="gender2"
					value="woman" ${dto.gender == 'woman' ? 'checked="checked"' : ''}>
			</div>
		</div>
		<div class="mb-3 row">
			...
			<div class="col-7" style="text-align: left;">
				<label for="checkbox1" class="form-check-label">정치</label> <input
					...
					checked="${inters[0] == 'pol' ? checked : false}">
			</div>
		...
		<div class="mb-3 row">
			...
			<div class="col-7">
				<select class="form-select" name="education">
					...
					<option value="ele" ${dto.education == 'ele' ? 'selected' : ''}>초등학교</option>
					<option value="mid" ${dto.education == 'mid' ? 'selected' : ''}>중학교</option>
					<option value="hig" ${dto.education == 'hig' ? 'selected' : ''}>고등학교</option>
					<option value="uni" ${dto.education == 'uni' ? 'selected' : ''}>대학교</option>
				</select>
			</div>
		</div>
		<div class="mb-3 row">
			...
			<div class="col-7">
				<textarea class="form-control" name="introduction" id="area"
					rows="3">${dto.introduction}</textarea>
			</div>
		</div>
		<div class="mb-3 row">
			<div class="offset-4 col-7" style="text-align: left;">
...
<script src="<c:url value="/js/MemberEditForm.js"/>"></script>